...
- **analyzer** (string, _optional_): name of an [Analyzer](../search-analyzers.html). Uses the Analyzer of a wrapping `ANALYZER()` call if not specified or defaults to `"identity"`
- returns nothing: the function can only be called in a [SEARCH operation](../operations/search) and throws an error otherwise
...
- **analyzer** (string): name of an [Analyzer](../search-analyzers.html).
